...
class MyOrders extends Component {
            ...
            if (snapshot.val()) {
                ...
                Object.keys(snapshot.val()).forEach((key) => {
                    promises.push(firebase.database().ref(`/orders/${key}`).once('value').then((orderSnapshot) => {
                        ...
                        // let count = 0;
                        // firebase.database().ref(`/vendor-orders/${order.vendorId}`).on('value', (snapshot) => {
                        //     if (snapshot.val()) {
                        //         Object.keys(snapshot.val()).forEach((key) => {
                        //             if (key != order.id) {
                        //                 count++;
    ...
    handleActionSelect = (index, selectedItem) => {
        ...
        if (index === 0 && status === Status.NOT_READY) {
            ...
            // console.log("item TIMESTAMP:" + time)
            if (
                ...
                Toast.show({
                    text: `You can't remove the order longer than 2 minutes`,
                    position: 'bottom',
                    duration: 5000
                })
            }
    ...
    render() {
            ...
            return ( <
                    ...
                    Content contentContainerStyle = {
                        ...
                        orders = {
                            this.state.orders.filter((order) => order.status !== Status.PICKED_UP)
                        }
                        vendor = {
                            false
                        }
                        ...
                        /View> :
                            ...
                            Text style = {
                                styles.section
                            } > You have no active orders right now. < /Text> <
                            Button style = {
                                [{
                                    alignSelf: 'center'
                                }, styles.section]
